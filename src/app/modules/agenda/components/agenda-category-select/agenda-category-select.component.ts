...
import {
  ...
  termsMatch
} from '../../../news/utils/news-taxonomy.util';

@Component({
  selector: 'app-agenda-category-select',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './agenda-category-select.component.html',
  styleUrls: ['./agenda-category-select.component.scss']
})
export class AgendaCategorySelectComponent implements OnInit {
  ...
  ensureCatalogLoaded(): void {
    ...
    this.loading = true;
    this.agendaService.getCategories().subscribe({
      next: cats => {
