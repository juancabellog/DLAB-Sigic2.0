...
@Component({
  selector: 'app-scientific-impact-report',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './scientific-impact-report.component.html',
  styleUrls: ['./scientific-impact-report.component.scss']
})
export class ScientificImpactReportComponent implements OnInit {
  loading = false;
  generating = false;
  ...
  generateReport(): void {
    ...
    const request = { idReport: 3 }; // Scientific Impact Report
    ...
    this.http.post(`${apiUrl}/analysis/generate-report`, request, {
      ...
      next: (response) => {
        ...
        link.href = url;
        link.download = 'scientific-impact-report.xlsx';
